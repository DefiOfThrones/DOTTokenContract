...
contract DoTxKnightNFT is ERC721, Ownable, ERC721Burnable {

    uint256 public nextDoTxId;
    ...
    event NewDoTx(uint256 id);
    event TransferDoTxNFT(address from, address to, uint256 tokenId);
    

    constructor(address _minterAddress, address _minterContractAddress, string memory _baseUrl) public ERC721("DoTxNFT", "DoTxNFT"){
        setMinter(_minterAddress);
        setMinterContract(_minterContractAddress);
        setBaseURI(_baseUrl);
    }
    
    function mintDoTx(address _to, string memory _collection, int betProtected, int rewardMalus, uint256 requiredLevel, string memory category, string memory itemType) public onlyOwnerOrMinter {
        nextDoTxId++;
                
        setCollectionForToken(nextDoTxId, _collection);
        tokenInfo[nextDoTxId].tokenId = nextDoTxId;
        fillItemInfo(nextDoTxId, betProtected, rewardMalus, requiredLevel, category, itemType);
    ...
    */
    ...
    function _transfer(address from, address to, uint256 tokenId) internal virtual override {
        ...
        emit TransferDoTxNFT(from, to, tokenId);
    }
